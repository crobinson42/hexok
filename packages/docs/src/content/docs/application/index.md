...
`kerf/app` is the use-case layer. An `ApiUseCase` is a request/response use case. An `EventUseCase` handles a catalog event. Both are ordinary classes with typed statics.
