...
`hexok/core` is small on purpose. Entities throw a `CodedError`. `validate` returns a `Result`. Use cases throw a `CodedError` whose keys come from an `ErrorMap`. Schemas follow Standard Schema V1.
