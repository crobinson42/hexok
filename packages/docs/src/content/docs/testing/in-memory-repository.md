...
`InMemoryRepository.of` fakes a [CrudRepository](/domain/crud-repository/) port `{ get, save, list, delete }`. It clones on get/save so tests do not mutate the store by accident.
