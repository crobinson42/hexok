...
describe('App completeness', () => {
  it('types build as a missing-port message when Clock is omitted', () => {
    ...
    expectTypeOf(
      builder.build,
    ).toEqualTypeOf<`hexok: unprovided port "clock" (used by incident.close). Call .provide(token, impl) before .build()`>();
  });
  ...
  it('joins use-case keys when one missing port is used by several use cases', () => {
    ...
    expectTypeOf(
      builder.build,
    ).toEqualTypeOf<`hexok: unprovided port "clock" (used by incident.close, clock.stamp). Call .provide(token, impl) before .build()`>();
  });
  ...
  it('names each missing port when several are omitted', () => {
    ...
    expectTypeOf(builder.build).toEqualTypeOf<
      | `hexok: unprovided port "clock" (used by incident.close). Call .provide(token, impl) before .build()`
      | `hexok: unprovided port "incidents" (used by incident.close). Call .provide(token, impl) before .build()`
    >();
  });
  ...
  it('types build as a missing-catalog message when DomainEvents is unbound', () => {
    ...
    expectTypeOf(
      builder.build,
    ).toEqualTypeOf<`hexok: unbound catalog "domain" (used by incident.close). Call .bind(...) before .build()`>();
  });
  ...
  it('throws at runtime when a port is missing', () => {
    ...
    ).toThrow('hexok: unprovided port "Clock" (used by incident.close)');
  });

  it('names duplicate provide/bind', () => {
    expectTypeOf<DuplicatePortError>().toEqualTypeOf<`hexok: port already provided`>();
    expectTypeOf<
...
describe('App invoke', () => {
  ...
  it('start() twice throws; stop() is idempotent', async () => {
    const app = App.from({ close: CloseIncident })
      ...
      .build();
    await app.start();
    await expect(app.start()).rejects.toThrow('hexok: start() called twice');
    await app.stop();
    await app.stop();
    await app.start();
    await app.stop();
