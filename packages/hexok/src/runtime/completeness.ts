...
type MissingPortMessages<Bag extends UseCaseBag, Provided> =
  Exclude<RequiredPorts<Bag>, Provided> extends infer T
    ? T extends unknown
      ? `hexok: unprovided port "${PortDisplayName<Bag, T>}" (used by ${JoinUnion<UsedByPort<Bag, T>>}). Call .provide(token, impl) before .build()`
      : never
    : never;
...
type MissingCatalogMessages<Bag extends UseCaseBag, Bound> =
  Exclude<RequiredCatalogs<Bag>, Bound> extends infer T
    ? T extends unknown
      ? `hexok: unbound catalog "${CatalogKeyOf<T>}" (used by ${JoinUnion<UsedByCatalog<Bag, T>>}). Call .bind(...) before .build()`
      : never
    : never;
