...
type OnInCatalog<C> = C extends {
  ...
  ? [On] extends [Events]
    ? C
    : `kerf: EventUseCase "${Key}" static on is not in catalog`
  : C;

/**
 * Per-entry diagnostic for `App.from` / `App.test`. Missing statics become a
 * `kerf:` sentence instead of a UseCaseClass union dump.
 */
export type CheckUseCase<C> = C extends { trigger: 'api' }
  ? C extends { ports: Record<string, PortToken<unknown>> }
    ? C
    : `kerf: ApiUseCase "${KeyOf<C>}" is missing static ports`
  : C extends { trigger: 'event' }
    ? C extends { on: { readonly key: string } }
      ? C extends { catalog: AnyEventCatalog }
        ? OnInCatalog<C>
        : `kerf: EventUseCase "${KeyOf<C>}" is missing static catalog`
      : `kerf: EventUseCase "${KeyOf<C>}" is missing static on`
    : `kerf: "${KeyOf<C>}" must extend ApiUseCase or EventUseCase`;
