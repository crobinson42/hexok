...
> {
  ...
  ctx<C>(): EventCatalog<Key, Kind, Events, C> {
    this.assertWritable();
    if (this.#hasCtx) {
      throw new Error(`kerf: catalog "${this.key}" already has ctx`);
    }
    if (this.#events.size > 0) {
      throw new Error(
        `kerf: catalog "${this.key}" ctx() must be called before .event()`,
      );
    }
    this.#hasCtx = true;
  ...
  event<E extends EventClass>(
    ...
    if (this.#events.has(eventKey)) {
      throw new Error(
        `kerf: duplicate event "${eventKey}" in catalog "${this.key}"`,
      );
    }
  ...
  get<E extends EventClass>(eventClass: E): E {
    ...
    if (!found) {
      throw new Error(
        `kerf: event "${eventClass.key}" is not in catalog "${this.key}"`,
      );
    }
    return found as E;
  }
  ...
  private assertWritable(): void {
    if (this.#frozen) {
      throw new Error(`kerf: catalog "${this.key}" is frozen`);
    }
  }
}
