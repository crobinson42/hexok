...
> {
  ...
  provide<I>(
    ...
    if (this.#provided.has(port as PortToken<unknown>)) {
      throw new Error(`kerf: port "${port.key}" already provided`);
    }
    for (const existing of this.#provided.keys()) {
      if (existing.key === port.key && existing !== port) {
        throw new Error(`kerf: two tokens share the key "${port.key}"`);
      }
    }
  ...
  > {
    ...
    if (this.#bound.has(cat as AnyEventCatalog)) {
      throw new Error(`kerf: catalog "${cat.key}" already bound`);
    }
    if (adapter.kind !== cat.kind) {
      throw new Error(
        `kerf: catalog "${cat.key}" is kind "${cat.kind}" but the adapter is "${adapter.kind}"`,
      );
    }
    cat.freeze();
  ...
  intercept(interceptor: Interceptor): this {
    if (this.#interceptors.some((item) => item.key === interceptor.key)) {
      throw new Error(`kerf: duplicate interceptor key "${interceptor.key}"`);
    }
  ...
  #build(): AppInstance<Bag, Ctx> {
    ...
    const instance: AppInstance<Bag, Ctx> = {
      ...
      start: async () => {
        if (started) throw new Error('kerf: start() called twice');
        started = true;
  ...
  #assertComplete(): void {
    ...
    for (const { token, usedBy } of requiredPorts.values()) {
      if (!this.#provided.has(token)) {
        throw new Error(
          `kerf: unprovided port "${token.key}" (used by ${usedBy.join(', ')})`,
        );
      }
    }
    for (const { catalog, usedBy } of requiredCatalogs.values()) {
      if (!this.#bound.has(catalog)) {
        throw new Error(
          `kerf: unbound catalog "${catalog.key}" (used by ${usedBy.join(', ')})`,
        );
      }
    }
  }
