...
type MissingPortMessages<Bag extends UseCaseBag, Provided> =
  Exclude<RequiredPorts<Bag>, Provided> extends infer T
    ? T extends unknown
      ? `kerf: unprovided port "${PortDisplayName<Bag, T>}" (used by ${JoinUnion<UsedByPort<Bag, T>>}). Call .provide(token, impl) before .build()`
      : never
    : never;
...
type MissingCatalogMessages<Bag extends UseCaseBag, Bound> =
  Exclude<RequiredCatalogs<Bag>, Bound> extends infer T
    ? T extends unknown
      ? `kerf: unbound catalog "${CatalogKeyOf<T>}" (used by ${JoinUnion<UsedByCatalog<Bag, T>>}). Call .bind(...) before .build()`
      : never
    : never;
...
export type DuplicatePortError = `kerf: port already provided`;

export type DuplicateCatalogError<N extends string = string> = string extends N
  ? `kerf: catalog already bound`
  : `kerf: catalog "${N}" already bound`;
