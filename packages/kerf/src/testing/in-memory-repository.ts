...
type AssertCrud<I> = 'get' | 'save' extends keyof I
  ? ExtraKeys<I> extends never
    ? I
    : `kerf: InMemoryRepository cannot fake extra methods; write a custom fake`
  : `kerf: InMemoryRepository.of expects a CRUD repository port`;
  ...
  implements CrudRepository<E>
{
  ...
  private keyOf(entity: E): string {
    ...
    if (typeof key !== 'string') {
      throw new Error(
        `kerf: InMemoryRepository keyBy "${this.#keyBy}" is not a string`,
      );
    }
    return key;
  }
